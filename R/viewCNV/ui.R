...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      tags$table(tags$tr(tags$td(selectInput("saved.site","Saved Sites",list(),width='250px'), valign="middle"),
                         tags$td(actionButton("saved.prev","Prev"), valign="middle"),
                         tags$td(actionButton("saved.next","Next"), valign="middle"))),
      actionButton("reset","Reset"),actionButton("delete","Delete"),
      actionButton("save.button","Save Site"),
      tags$table(tags$tr(tags$td(selectInput("candidate","GenomeStrip CNVs", list(), width='250px')),
                         tags$td(actionButton("candidate.prev","Prev")),
                         tags$td(actionButton("candidate.next","Next")))),
      hr(),
      tags$table(tags$tr(tags$td(textInput("seg.chr","Chromosome",NA), valign="bottom"),
                         tags$td(numericInput("seg.start","Start Position", NA),valign="bottom"),
                         tags$td(numericInput("seg.end", "End Position", NA),valign="bottom"),
                         tags$td(textOutput("seg.size")),
                         tags$td(actionButton("zoom.out", "Zoom Out"),valign="middle"))),
